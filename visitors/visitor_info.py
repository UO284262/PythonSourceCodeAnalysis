...
class Visitor_info(NodeVisitor):
    ...
    def visit_Program(self: Self, params : Dict):
                        ...
                        if module_ast:
                            try:
                                if(len(module_ast.body) > 0):
                                    modules.append(self.visit(module_ast, {"program_id" : id, "user_id" : params["user_id"], "expertise_level" : params["expertise_level"], "filename" : archivo.split('.')[0], "path" : ruta_completa}))
        ...
        if(index > 0):
            ...
            dbprogram.expertise_level = params["expertise_level"]
    ...
    def visit_Module(self : Self, node : ast.Module , params : Dict) -> Dict: 
        ...
        ############# PARAMS #####################
        childparams = {'expertise_level' : params["expertise_level"], 'user_id' : params['user_id'], "parent" : module, "depth" : 1, "parent_id" : id, "role" : "Module"}
        ############## PROPAGAR VISIT ############
        methodCount = 0
        functionsBodySize = 0
    ...
    def visit_FunctionDef(self : Self, node : ast.FunctionDef , params : Dict) -> Dict: 
        ...
        ############# PARAMS #####################
        childparams = {'expertise_level' : params["expertise_level"], 'user_id' : params['user_id'], "parent" : function, "depth" : params["depth"] + 1, "parent_id" : id}
        if(isMethod):
            stmtRoles = ["MethodDef"]
    ...
    def visit_Return(self : Self, node : ast.Return , params : Dict) -> Dict: 
        ...
        ############# PARAMS #####################
        childparams = {'expertise_level' : params["expertise_level"], 'user_id' : params['user_id'], "parent" : stmt, "depth" : params["depth"] + 1, "parent_id" : id, "role" : "Return"}
        ########## ENTITIE PROPERTIES ############
        returns = None
        ############## PROPAGAR VISIT ############
    ...
    def visit_Delete(self : Self, node : ast.Delete , params : Dict) -> Dict: 
        ...
        stmt.statementRole = params["role"]
        ############# PARAMS #####################
        childparams = {'expertise_level' : params["expertise_level"], 'user_id' : params['user_id'], "parent" : stmt, "depth" : params["depth"] + 1, "parent_id" : id, "role" : "Delete"}
        ########## ENTITIE PROPERTIES ############
        depth = 0
        first_child_id = None
    ...
    def visit_MatchValue(self : Self, node : ast.MatchValue , params : Dict) -> Dict: 
        childparams = {'expertise_level' : params["expertise_level"], 'user_id' : params['user_id'], "parent" : params["parent"], "depth" : params["depth"] + 1, "parent_id" : params["parent_id"]}
        exprRoles = ["MatchCondition"]
        ################ RETURNS #################
        depth = 0
        ############## PROPAGAR VISIT ############
    ...
    def visit_ExceptHandler(self : Self, node : ast.ExceptHandler , params : Dict) -> Dict: 
        ############# PARAMS #####################
        childparams = {'expertise_level' : params["expertise_level"], 'user_id' : params['user_id'], "parent" : params["handler"], "depth" : params["depth"] + 1, "parent_id" : params["parent_id"]}
        ...
        for child in node.body:
            ...
            depth = max(depth,returns[index]["depth"])
            index += 1
        return { 'id' : params["parent_id"], 'depth' : depth, 'isCatchAll' : isCatchAll, 'child_ids' : child_ids}
    ...
    def visit_arguments(self : Self, node : ast.arguments , params : Dict) -> Dict: 
        ...
        dbparams.expertise_level = params['expertise_level']
    ...
    def visit_withitem(self : Self, node : ast.withitem , params : Dict) -> Dict: 
        ############# PARAMS #####################
        childparams = {'expertise_level' : params["expertise_level"], 'user_id' : params['user_id'], "parent" : params["parent"], "depth" : params["depth"], "parent_id" : params["parent_id"]}
        ############## PROPAGAR VISIT ############
